...
def load_data_semseg(partition, test_area):
    ...
    if partition == 'train':
        ...
        data_dir = os.path.join(DATA_DIR, 's3dis-hdf5-test')

    with open(os.path.join(DATA_DIR, 's3dis-hdf5-test', 'dict_max_values.pickle'), 'rb') as handle:
    ...
    for i, room_name in enumerate(room_filelist):
        if test_area_name in room_name:
            test_idxs.append(i)
            max_values.append(dict_max_values[room_name])
        else:
            train_idxs.append(i)
    if partition == 'train':
    ...
    return all_data, all_seg, max_values


def load_data_semseg_arch(partition, test_area):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'data')
    META_DIR = os.path.join(DATA_DIR, 'arch', 'meta')
    class_names = []
    ...
    # prepare_test_data_semseg()
    if partition == 'train':
        data_dir = os.path.join(DATA_DIR, 'arch-hdf5-train')
    else:
        data_dir = os.path.join(DATA_DIR, 'arch-hdf5-test')

    with open(os.path.join(DATA_DIR, 'arch-hdf5-test', 'dict_max_values.pickle'), 'rb') as handle:
        dict_max_values = pickle.load(handle)
    ...
    data_batchlist, label_batchlist = [], []
    for f in all_files:
        file = h5py.File(os.path.join(DATA_DIR, f).replace("\\", "/"), 'r+')
        data = file["data"][:]
        label = file["label"][:]
        data_batchlist.append(data)
        label_batchlist.append(label)
    ...
    for i, room_name in enumerate(room_filelist):
        if test_area_name in room_name:
            test_idxs.append(i)
            # max_values.append(dict_max_values[test_area_name])
            max_values.append(dict_max_values[test_area_name + "_" + test_area_name])
        else:
            train_idxs.append(i)
    if partition == 'train':
        all_data = data_batches[train_idxs, ...]
    ...
    return all_data, all_seg, max_values, class_names


def load_data_semseg_arch9l(partition, test_area):
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'data')
    META_DIR = os.path.join(DATA_DIR, 'arch9l', 'meta')
    class_names = []
    with open(os.path.join(META_DIR, "class_names.txt"), "r") as fr:
        for l in fr:
            class_names.append(l.strip())
    ...
    # prepare_test_data_semseg()
    if partition == 'train':
        data_dir = os.path.join(DATA_DIR, 'arch9l-hdf5-train')
    else:
        data_dir = os.path.join(DATA_DIR, 'arch9l-hdf5-test')

    with open(os.path.join(DATA_DIR, 'arch9l-hdf5-test', 'dict_max_values.pickle'), 'rb') as handle:
        dict_max_values = pickle.load(handle)
...
def load_data_semseg_sinthcity(partition, test_area):
    ...
    # prepare_test_data_semseg()
    if partition == 'train':
        data_dir = os.path.join(DATA_DIR, 'sinthcity-hdf5')
    else:
        data_dir = os.path.join(DATA_DIR, 'sinthcity-hdf5')

    with open(os.path.join(DATA_DIR, 'sinthcity-hdf5', 'dict_max_values.pickle'), 'rb') as handle:
        dict_max_values = pickle.load(handle)
...
class ScanObjectNN(Dataset):
    def __init__(self, num_points, partition='train'):
        self.data, self.label = load_data_cls_(partition)
...
def load_data_cls_(partition):
    #download_scanObjectNN()
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DATA_DIR = os.path.join(BASE_DIR, 'data')

    h5_name = os.path.join(DATA_DIR, 'scanObjectNN', partition + "_objectdataset.h5")
    f = h5py.File(h5_name, 'r+')
    data = f['data'][:].astype('float32')
    label = f['label'][:][:,np.newaxis].astype('int64')
    f.close()
...
class S3DIS(Dataset):
    def __init__(self, num_points=4096, partition='train', test_area='1'):
        self.data, self.seg, self.max = load_data_semseg(partition, test_area)
        self.num_points = num_points
        self.partition = partition
    ...
    def __getitem__(self, item):
        ...
        if self.partition == 'train':
            ...
            max = self.max[item][:self.num_points]
            seg = torch.LongTensor(seg)
            return pointcloud, seg, max
...
class ArCH(Dataset):
    def __init__(self, num_points=4096, partition='train', test_area='1', tipo="all"):
        if tipo == "all":
            self.data, self.seg, self.max, self.class_names = load_data_semseg_arch(partition, test_area)
        elif tipo == "9l":
            self.data, self.seg, self.max, self.class_names = load_data_semseg_arch9l(partition, test_area)
        self.num_points = num_points
        self.partition = partition
...
class Sinthcity(Dataset):
    def __init__(self, num_points=4096, partition='train', test_area='1'):
        self.data, self.seg, self.max, self.class_names = load_data_semseg_sinthcity(partition, test_area)
        self.num_points = num_points
        self.partition = partition
